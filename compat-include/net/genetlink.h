...
#define genl_info_net(info) ovpn_genl_info_net((info))

#endif /* LINUX_VERSION_CODE < KERNEL_VERSION(6, 5, 0) */

#if LINUX_VERSION_CODE < KERNEL_VERSION(6, 9, 0)

#undef GENL_REQ_ATTR_CHECK
#define GENL_REQ_ATTR_CHECK(info, attr)                                \
	({                                                             \
		struct genl_info *__info = (struct genl_info *)(info); \
                                                                       \
		NL_REQ_ATTR_CHECK(__info->extack, NULL, __info->attrs, \
				  (attr));                             \
	})
